...
void solve()
{
	...
	rdp[n - k].ss = n - k + 1;
	for (i = n - k - 1; i >= 0; i--) {
		if (rdp[i + 1].ff > (suf[i] - suf[i + k]))
		{
			rdp[i].ff = rdp[i + 1].ff;
			rdp[i].ss = rdp[i + 1].ss;
		}
		else {
			rdp[i].ff = suf[i] - suf[i + k];
			rdp[i].ss = i + 1;
		}
	}
	// for (i = 0; i < n; i++) {
	...
	// }
	// cout << endl;
	ans = pre[k - 1] + rdp[k].ff;
	ll l1 = -1, l2 = -1;
	l1 = 1;
	l2 = rdp[k].ss;
	for (i = k; i <= n - k - 1; i++) {
		// cout << ans << endl;
		if (ans < (pre[i] - pre[i - k] + rdp[i + 1].ff)) {
			ans = (pre[i] - pre[i - k] + rdp[i + 1].ff);
			l1 = i - k + 2;
			l2 = rdp[i + 1].ss;
			// ans = max(ans, pre[i] - pre[max(i - k, 0ll)] + rdp[i + 1]);
		}
	}
	// cout << ans << endl;
